...
<header>
  <div class="bg text-white py-5 masthead shadow p-3">
    <div class="container py-5">
      ...
      <button @click="openChat" type="button" class="btn shadow-0 text-white pt-2 category-change" style="background-color: #E73C17; margin-left: 3px">
...
<section>
    <div class="container my-5">
      <header class="mb-4">
        <h3 style="text-align: left; color: #525252">Telefonía con envío gratis <span style="color: #E73C17; font-size: 20px"><i class="bi bi-telephone"></i>
</span></h3>
      </header>
      <div class="row">
        <div v-for="(product, index) in freeShippingProducts" :key="index" class="col-lg-3 col-md-6 col-sm-6 d-flex">
          <div class="card w-100 my-2 shadow-2-strong">
              ...
              <img @click="checkProductDetail(product)" :src="product.picture" class="card-img-top mx-auto mt-3" style="aspect-ratio: 1 / 1; width: 50%; height: auto" />
  ...
  <section>
    <div class="container my-5">
      <header class="mb-4">
        <h3 style="text-align: left; color: #525252">Descuento en videojuegos <span style="color: #E73C17; font-size: 20px"><i class="bi bi-controller"></i>
</span></h3>
      </header>
      <div class="row">
        <div v-for="(product, index) in consoleDiscountProducts" :key="index" class="col-lg-3 col-md-6 col-sm-6 d-flex">
          <div class="card w-100 my-2 shadow-2-strong">
            ...
            <div class="card-body d-flex flex-column" style="max-height: 300px;">
              ...
              <p class="card-text"> 
                <small class="text-muted">
                <strike>${{ product.originalPrice }} CLP</strike>
                </small>
              </p>
              <div class="card-footer d-flex align-items-end justify-content-center pt-3 px-0 pb-0 mt-auto">
                <a :href="product.permalink" target="_blank" class="btn category-change shadow-0 me-1 text-white shop">Ver en tienda</a>
...
export default {
  ...
  methods :{
    ...
    async getProducts(){
      await axios.get(this.freeShippingUrl)
      .then((response =>{
        return this.freeShippingProducts = response.data;
      }))
      .catch((err =>{
        console.log(err);
      })); 
    ...
    async getDiscountProducts(){
      await axios.get(this.consoleDiscountUrl)
      .then((response =>{
        return this.consoleDiscountProducts = response.data;
      }))
      .catch((err =>{
        console.log(err);
      })); 
